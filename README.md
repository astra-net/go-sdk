...
This is a go layer on top of the Astra RPC, included is a CLI tool that you can build with a
simple invocation of `make`

See https://docs.astranetwork.com/home/network/wallets/astra-cli/download-setup for detailed
documentation on how to use the `astra` CLI tools
...
```
$ docker run -it  astra-network/main:stable /bin/bash
$ cd ../go-sdk
...
`astra` implements a fluent API, that is, there is a hierarchy of commands.

# bash completions

once built, add `astra` to your path and add to your `.bashrc`

```
. <(astra completion)
```

invoke the following command to see the most command usages of `astra`
...
./astra --node=https://rpc.s0.m.astranetwork.com balances <SOME_ADDRESS>

2.  Check sent transaction
./astra --node=https://rpc.s0.m.astranetwork.com blockchain transaction-by-hash <SOME_TX_HASH>

3.  List local account keys
./astra keys list

4.  Sending a transaction (waits 40 seconds for transaction confirmation)
./astra --node=https://rpc.s0.m.astranetwork.com transfer \
...
./astra --node=https://rpc.s0.m.astranetwork.com transfer --file <PATH_TO_JSON_FILE>
Check README for details on json file format.

6.  Check a completed transaction receipt
./astra --node=https://rpc.s0.m.astranetwork.com blockchain transaction-receipt <SOME_TX_HASH>

7.  Import an account using the mnemonic. Prompts the user to give the mnemonic.
./astra keys recover-from-mnemonic <ACCOUNT_NAME>
...
./astra --node=https://rpc.s0.m.astranetwork.com staking create-validator --amount 10 --validator-addr <SOME_ADDRESS> \
...
./astra --node=https://rpc.s0.m.astranetwork.com staking edit-validator \
...
./astra --node=https://rpc.s0.m.astranetwork.com staking delegate \
    --delegator-addr <SOME_ADDRESS> --validator-addr <VALIDATOR_ADDRESS> \
    --amount 10 --passphrase

15. Undelegate to a validator
./astra --node=https://rpc.s0.m.astranetwork.com staking undelegate \
    --delegator-addr <SOME_ADDRESS> --validator-addr <VALIDATOR_ADDRESS> \
    --amount 10 --passphrase

16. Collect block rewards as a delegator
./astra --node=https://rpc.s0.m.astranetwork.com staking collect-rewards \
    --delegator-addr <SOME_ADDRESS> --passphrase

17. Check elected validators
./astra --node=https://rpc.s0.m.astranetwork.com blockchain validator elected

18. Get current staking utility metrics
./astra --node=https://rpc.s0.m.astranetwork.com blockchain utility-metrics

19. Check in-memory record of failed staking transactions
./astra --node=https://rpc.s0.m.astranetwork.com failures staking

20. Check which shard your BLS public key would be assigned to as a validator
./astra --node=https://rpc.s0.m.astranetwork.com utility shard-for-bls <BLS_PUBLIC_KEY>
...
./astra governance new-proposal --proposal-yaml=[file path] --key=[key name]
...
./astra governance vote-proposal --proposal=[proposal hash] --choice=[your choice text, eg: yes] --key=[key name]
PS: key must first use (astra keys import-private-key) to import

26. Enter Console
./astra command --net=testnet --shard=0
```
...
```
astra --node="https://rpc.s1.m.astranetwork.com/" transfer --file ./batchTransactions.json
```
...
|        `to`         |   string   | [**Required**] The receivers address.                                                                                                  |
...
```bash
./astra get-nonce --node=https://rpc.s0.m.astranetwork.com --from=[ADDRESS]
```

2. Sign transfer and write to file. (Passphrase required, But no need to be online)

```bash
./astra transfer --offline-sign --nonce=[nonce value from previous] --from=[ADDRESS] --to=[ADDRESS] --amount=1000 --from-shard=0 --to-shard=0 > signed.json
```

3. send `signed.json` to Astra blockchain! (Need to be online, but no passphrase required)

```bash
./astra offline-sign-transfer --node=https://rpc.s0.t.astranetwork.com --file ./signed.json
```

# Debugging

The go-sdk code respects `ASTRA_RPC_DEBUG ASTRA_TX_DEBUG` as debugging
based environment variables.

```bash
ASTRA_RPC_DEBUG=true ASTRA_TX_DEBUG=true ./astra blockchain protocol-version
```
...
```bash
./astra command --node="https://rpc.s0.t.astranetwork.com" --net=testnet
```
...
var code =
  "";
var account = "one1xjanr7lgulc0fqyc8dmfp6jfwuje2d94xfnzyd"; // change this to your account address
